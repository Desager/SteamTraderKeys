...
internal class InitSteamTraderDataActor(
    ...
    override fun process(commandFlow: Flow<TraderCommand>): Flow<TraderEvent> {
        return commandFlow.filterIsInstance<TraderCommand.InitSteamTraderData>()
            .flatMapLatest {
                Log.d(TAG, "InitSteamTraderData received")
                steamTraderGetRepository.get()
            }
            .mapLatest { TraderEvent.Internal.SteamTraderGetDataStatus(it) }
    }

    companion object {
        private const val TAG = "InitSteamTraderDataActor"
    }
}